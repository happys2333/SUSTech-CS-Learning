...
\usepackage{fancybox,mathrsfs,version,bm}
...
\newtheorem{exercise}{\heiti 练习}[chapter]
\newtheorem{assumption}{\heiti 假设}[chapter]
\newtheorem{axiom}{\heiti 公理}[chapter]
\newtheorem{property}{\heiti 性质}[chapter]
\newtheorem{conjecture}{\heiti 猜想}[chapter]
...
%---------- 定义章节标题格式 ------------
\titleformat{\chapter}[hang]{\heiti \chpzihao \filcenter}
    {\chpzihao \chaptername}{20pt}{\chpzihao}
...
\setlength{\parskip}{4pt plus1pt minus1pt}   % 段落之间的竖直距离
...
%---------使用默认封面 -----------
%\title{\huge\heiti 这是一个书籍的 \LaTeX ~ 模板}
%\author{\Large\fangsong 某某某~~编著}
%\date{2021年3月}
...
\chapter{自序}
这本书是我在新冠疫情期间回忆自己的学习经历的梳理与总结，之所以要完成这一本手册，是因为我本身是一名从计算机零基础入门的学生。在学习过程中，我曾有一段时间的迷茫期，和那些高中就有很深计算机功底的同学们相比，我的计算机知识少的可怜。这让我十分希望有一本资料可以让我快速的学习相关的知识，让我可以“听懂”其他同学们和老师交流的内容。我相信有这个希望的同学们不会在少数，于是便诞生了编写这么本资料的想法。本书希望可以帮助几乎没有计算机编程基础的小白入门并掌握计算机的基本知识，快速跟上学校的计算机课程，并逐渐掌握学习的主动权。本书也希望可以引导大家学到更加高级实用的内容，所以如果您已经有较好的Java基础，可以选择跳过Java入门章节，直接学习后面的其他内容。如果您是一名刚刚入门计算机或者准备入门计算机的朋友，那么本书将会让你从0基础慢慢的走入计算机的大门。这本书我将尽可能的涵盖计算机学习中我自己学习到的知识与技能，帮助你快速无痛苦的掌握计算机学习的主动权。注意：我建议你在学习的时候要以自己的需求为主，不要按照顺序学习，要按照自己想学的知识进行跳读，这样可以帮助你事半功倍。
...
常用软件，意味着在学习中或以后工作之中或多或少都会需要使用的一部分软件，对于如何使用软件，大部分同学实际上是有很多问题的，有些同学经常会在JavaA课程学完后仍然对idea等软件使用不够熟练，本章节的目的是教授常见软件的基本使用方法和一部分常用的功能使用。本章最后一节将会推荐部分我平时使用的软件，这些软件在不同情况下都会有自己的使用优势，可以自行选择是否要下载安装。


\section{JetBrains(IDEA)}
对于任何学习计算机的同学或者甚至只要你需要学Java课程，那么JetBrains的软件是你必不可少的，JetBrains的软件大同小异，这里我们将以IDEA这款软件讲解使用的基本技巧。\\
...
这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字.
...
参考文献采用 BibLaTeX 的方式生成 (内容写在文件 \verb|mybib.bib| 中), 参考文献的样式为 \verb|thuthesis-numeric|, 参考了 清华大学 LaTeX 模板 \href{https://github.com/tuna/thuthesis}{\fbox{thuthesis}} 的文献样式. 参考文献的样式还可以选择 BibLaTeX 的标准样式: \verb|plain|、\verb|abbrv|、\verb|unsrt| 与 \verb|siam| 等.
...
\left\{\begin{aligned}
& L_{h} u_{i}=-\frac{u_{i+1}-2 u_{i}+u_{i-1}}{h^{2}}+\frac{u_{i+1}-u_{i-1}}{2h}+q_{i} u_{i}=f_{i},\ i=1, 2, \cdots, N-1, \\
